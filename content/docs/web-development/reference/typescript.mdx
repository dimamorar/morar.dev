...
> 
> 
> This increases reliability, readability, and scalability of the project — especially in large teams.
> 
...
- **Better Refactoring and Code Navigation**
    ...
    Types provide checking of dependencies between modules — it's easy to change the structure without errors.
...
- **Readonly fields** — prohibit change after initialization.
...
- They act as "variables for types" — substituted during call or use.
...
> 
> 
> Used for convenient work with fixed sets of values — for example, statuses, roles, or directions.
> 
...
- Here, each element `"a" | "b" | "c"` is checked separately — this is called a **distributive conditional type**.
...
- Conditional types = **"if–else" type logic** for checking relationships between types.
...
- Union is suitable for **alternatives**, intersection — for **object composition**.
...
> 
> helping to implement **encapsulation** — one of the key principles of OOP.
> 
...
- `protected` → in the class and its descendants.
- Access modifiers are applied only at the compilation stage — **they do not exist in JavaScript**, but they ensure type safety and clear architecture.
...
- `implements` → **guarantees compliance with the contract**, but does not provide code.
...
abstract class Shape {
  abstract getArea(): number; // ❌ without implementation
...
> TypeScript uses structural typing — type compatibility is determined by the shape of the object, not by its name or origin.
> 
> 
...
Admin a = new User(); // ❌ not allowed without explicit inheritance
...
const anotherUser: User = admin; // ✅ allowed (User structure is a subset of Admin)
...
- TypeScript = **structural typing** → types are compatible by shape, not by name.
- Nominal typing = **nominal typing** → origin or specific declaration is important.
...
const b = new Button();
setTimeout(b.handleClick, 1000); // ❌ this = undefined in strict mode
```
...
- In regular functions, you can **explicitly specify the type of `this`** in the first parameter — it is not part of the arguments list.
...
{/* Image missing: image 1.png */}

`(n: string) ⇒ void` - void when it's not important what value the function outputs. even if in runtime it returns something.
...
{/* Image missing: image 3.png */}

{/* Image missing: image 4.png */}
...
{/* Image missing: image 9.png */}

{/* Image missing: image 10.png */}
