---
title: "Token Refresh Flow on Frontend"
description: "A mechanism for extending a user session without repeating the login process."
---
...
The Access Token is valid for a limited time (e.g., 15 minutes).

When it expires, the client requests a new one using the Refresh Token.

The update is performed automatically (silent refresh) or upon the first failed attempt to access the API.
